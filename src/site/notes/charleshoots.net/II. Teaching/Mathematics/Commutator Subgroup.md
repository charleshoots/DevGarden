...
Links: [[/II. Teaching/Mathematics/Normal Subgroups and Quotient Groups\|Normal Subgroups and Quotient Groups]], [[/II. Teaching/Mathematics/Subgroups\|Subgroups]], [[/II. Teaching/Mathematics/Cauchy and Sylow Theorems\|Cauchy and Sylow Theorems]]
...
\end{document}
```
Thus, $(-)^\text{ab}: \sf Grp \to Ab$ is a [[/II. Teaching/Mathematics/Categories and Functors\|covariant functor]], where $G \mapsto G^\text{ab}$, and $(f: G \to H) \mapsto f^\text{ab}: G^\text{ab} \to H^\text{ab}$. 

**Prop:** Let $\{G_\alpha \mid \alpha<\kappa\}$ be a collection of groups. Then $$\left(\coprod_{\alpha<\kappa} G_\alpha\right)^\text{ab} \cong \bigoplus_{\alpha <\kappa} G_\alpha^\text{ab}.$$This means that $(-)^\text{ab}$ sends the [[/II. Teaching/Mathematics/Free Product of Groups\|coproduct]] of $\sf Grp$ to the coproduct of the images in $\sf Ab$. 

**Cor:** Let $S$ be a nonempty set, then $F(S)^\text{ab} \cong \Bbb Z\langle S\rangle$, where $F(S)$ is the [free group](Free%20Groups) generated by $S$, and $\Bbb \langle S \rangle$ is the[[/II. Teaching/Mathematics/Free Abelian Groups\| free abelian group]] generated by $S$, or the free $\Bbb Z$-module generated by $S$. 
...
**Th:** Let $G$ be a group. $Z^m(G) = G$ iff $L_{m+1}(G)$. It follows that $G$ is [[/II. Teaching/Mathematics/Nilpotent Groups\|nilpotent]] of class $m$ iff $L_{m+1}(G) = 1$, and $m$ is the least integer that satisfies this. 
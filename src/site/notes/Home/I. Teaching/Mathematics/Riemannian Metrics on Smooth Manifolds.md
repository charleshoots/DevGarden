...
- Each point of $M$ is contained in the domain of a [[charleshoots.net/I. Teaching/Mathematics/Lie Derivative#Commuting Frames\|commuting orthonormal frame]].
...
**Prop:** The set of maps from $\Bbb R^n$ to itself given by the action of $\text E(n)$, the [[charleshoots.net/I. Teaching/Mathematics/Euclidean Group\|Euclidean Group]], on $\Bbb R^n$ is the full group of Riemannian isometries of $(\Bbb R^n, \bar g)$. 
...
**Def:** Suppose $\pi: N \to M$ is a smooth covering map. A *covering transformation*, or *[[charleshoots.net/I. Teaching/Mathematics/Automorphism Group of a Covering Map\|deck transformation]]* is a smooth map $\varphi: N \to N$ such that $\pi \circ \varphi = \pi$. If $g$ is Riemannian metric on $M$, then $h := \pi^* g$ is a Riemannian metric on $N$ that is invariant under all covering transformations. In this case $h$ is called a *covering metric*, and $\pi$ is called *Riemannian covering*. 

**Prop:** If $(M, g)$ is a Riemannian $n$-manifold with or without boundary, let $UM \subseteq TM$ be the subset $\{(x, v) \in TM \mid |v|_g = 1\}$, called the *unit tangent bundle of $M$*. Then  $UM$ is a smooth [[charleshoots.net/I. Teaching/Mathematics/Fibre Bundles on Smooth Manifolds\|fibre bundle]] over $M$ with model fibre $\Bbb S^{n-1}$. 